        ...
        if request.is_json:
            ...
            if app.is_running:
                asyncio.run_coroutine_threadsafe(
                    app.feed_update(app.build_update(update_json)), 
                    app.loop
                )
            return "OK", 200
        else:
...
if __name__ == "__main__":
    if not WEBHOOK_URL:
        logging.error("WEBHOOK_URL environment variable is not set. Cannot start in webhook mode.")
    else:
        port = int(os.environ.get("PORT", 8080))
        
        async def start_bot_and_set_webhook():
            await app.start()
        ...
        try:
            loop.run_until_complete(start_bot_and_set_webhook())
            
            logging.info(f"Starting Flask server on port {port}...")
            flask_app.run(host="0.0.0.0", port=port)
            
            logging.info("Flask server stopped. Stopping Pyrogram client.")
            loop.run_until_complete(app.stop())
            
        except KeyboardInterrupt:
            logging.info("Interrupted. Stopping client and server...")
            loop.run_until_complete(app.stop())
        except Exception as e:
