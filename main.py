...
def build_action_keyboard(chat_id, message_id, text_length, user_id):
    buttons = []
    if user_id == 5240873494:
...
async def handle_media(client, message: Message):
    ...
    if sent_message:
        try:
            keyboard = build_action_keyboard(sent_message.chat.id, sent_message.id, len(text), uid)
