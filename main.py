...
app_pyro = Client("media_transcriber", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
@app_pyro.on_message(filters.command("help") & filters.private)
async def help_command(client, message: Message):
    if not await ensure_joined(client, message):
        return
    await message.reply_text(HELP_MESSAGE)

@app_pyro.on_message(filters.command("lang") & filters.private)
...
@app_pyro.on_callback_query(filters.regex(r"^lang\|"))
...
@app_pyro.on_message(filters.command("mode") & filters.private)
...
@app_pyro.on_callback_query(filters.regex(r"^mode\|"))
...
@app_pyro.on_message(filters.private & filters.text)
...
@app_pyro.on_message(filters.private & (filters.audio | filters.voice | filters.video | filters.document))
async def handle_media(client, message: Message):
    ...
    if sent_message:
        try:
            ...
            logging.error("Failed to attach keyboard or init usage: %s", str(e))

@app_pyro.on_callback_query(filters.regex(r"^clean\|"))
...
@app_pyro.on_callback_query(filters.regex(r"^summarize\|"))
...
bot_tele = telebot.TeleBot(BOT_TOKEN)
app_flask = Flask(__name__)

@bot_tele.message_handler(commands=['admin'])
def handle_online(message):
    bot_tele.send_message(message.chat.id, "😂 (Webhook)")

@app_flask.route('/', methods=['POST'])
def webhook():
    if request.headers.get('content-type') == 'application/json':
        update = telebot.types.Update.de_json(request.data.decode('utf-8'))
        bot_tele.process_new_updates([update])
        return '', 200
    else:
        abort(403)
...
def set_wh():
    if not WEBHOOK_URL.startswith("http"):
        return "Webhook URL not configured (must be http/https)", 500
    try:
        bot_tele.set_webhook(url=WEBHOOK_URL)
        return f"Webhook set to {WEBHOOK_URL}", 200
    except Exception as e:
        return f"Error setting webhook: {e}", 500

@app_flask.route('/delete_webhook', methods=['GET'])
def del_wh():
    bot_tele.delete_webhook()
    return "Webhook deleted", 200

@app_flask.route('/alive', methods=['GET'])
def index():
    return "Bot is running (Flask/Webhook)", 200

def run_flask():
    logging.info("Starting Flask webhook thread...")
    try:
        ...
        if parsed_url.scheme in ('http', 'https') and parsed_url.netloc:
             ...
             logging.warning("WEBHOOK_URL is not valid. Cannot set webhook automatically.")
        app_flask.run(host="0.0.0.0", port=PORT)
    except Exception as e:
        logging.error(f"Flask/Webhook thread failed: {e}")

def start_pyrogram_with_retries():
    retry_delay = 5
    while True:
        try:
            logging.info("Starting Pyrogram client")
            app_pyro.run()
            logging.info("Pyrogram client stopped normally")
            break
        except FloodWait as fw:
            wait_seconds = getattr(fw, "x", None) or getattr(fw, "seconds", None) or 0
            try:
            ...
            if wait_seconds <= 0:
                wait_seconds = 60
            logging.error("FloodWait received. Waiting for %s seconds before retrying", str(wait_seconds))
            time.sleep(wait_seconds + 5)
            continue
        except Exception as e:
            logging.exception("Unhandled exception while running Pyrogram client")
            time.sleep(retry_delay)
            retry_delay = min(retry_delay * 2, 300)
            continue

if __name__ == "__main__":
    webhook_thread = threading.Thread(target=run_flask)
    webhook_thread.daemon = True
    webhook_thread.start()
    logging.info("Flask thread started")
    start_pyrogram_with_retries()
