...
# New constant for the welcome message
...
async def start(client, message: Message):
    ...
    for i, (label, code) in enumerate(LANGS, 1):
        # Callback data format: lang|CODE
...
async def language_callback_query(client, callback_query):
    ...
    # Edit the language selection message to the welcome message
...
async def handle_text(client, message: Message):
    ...
    # NOTE: The old language selection logic is removed as it's now handled by inline buttons.
    # The language selection is now handled by the language_callback_query function.
