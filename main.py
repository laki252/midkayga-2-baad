...
async def clean_up_callback(client, callback_query: CallbackQuery):
    ...
    await callback_query.answer("Cleaning up and translating...", show_alert=False) # Updated text
    await client.send_chat_action(chat_id, ChatAction.TYPING)
    try:
        loop = asyncio.get_event_loop()
        uid = callback_query.from_user.id
        lang_code = get_user_lang(uid, "en") # Get user's preferred language code
        
        # Modified instruction to include cleaning AND translation to the preferred language
        instruction = f"Clean and normalize this transcription. Remove ASR artifacts like [inaudible], repeated words, filler noises, timestamps, and incorrect punctuation. **Translate the final cleaned text into the language with code '{lang_code}'**. Produce a clean, well-punctuated, readable text in the target language. Do not add introductions or explanations."
        
        cleaned_text = await loop.run_in_executor(None, ask_gemini, stored_text, instruction)
        
        if not cleaned_text:
            await client.send_message(chat_id, "No cleaned and translated text returned.", reply_to_message_id=orig_msg_id)
            return
        ...
        if len(cleaned_text) > MAX_MESSAGE_CHUNK:
            if mode == "💬 Split messages":
                ...
                file_name = os.path.join(DOWNLOADS_DIR, "Cleaned_Translated.txt") # Updated filename
                with open(file_name, "w", encoding="utf-8") as f:
                    f.write(cleaned_text)
                await client.send_chat_action(chat_id, ChatAction.UPLOAD_DOCUMENT)
                await client.send_document(chat_id, file_name, caption="Cleaned and Translated Transcript", reply_to_message_id=orig_msg_id)
                os.remove(file_name)
        else:
        ...
        await client.send_message(chat_id, f"❌ Error during cleanup and translation: {e}", reply_to_message_id=orig_msg_id)
...
async def get_key_points_callback(client, callback_query: CallbackQuery):
    ...
    stored_text = stored.get("text")
    orig_msg_id = stored.get("origin")
    await callback_query.answer("Generating summary and translating...", show_alert=False) # Updated text
    await client.send_chat_action(chat_id, ChatAction.TYPING)
    try:
        loop = asyncio.get_event_loop()
        uid = callback_query.from_user.id
        ...
        # Modified instruction to include translation to the preferred language
        instruction = f"What is this report and what is it about? Please summarize them for me and **translate the summary into the language with code '{lang_code}'**. Do not add any introductions, notes, or extra phrases."
        ...
        if len(summary) > MAX_MESSAGE_CHUNK:
            if mode == "💬 Split messages":
                ...
                file_name = os.path.join(DOWNLOADS_DIR, "Summary_Translated.txt") # Updated filename
                with open(file_name, "w", encoding="utf-8") as f:
                    f.write(summary)
                await client.send_chat_action(chat_id, ChatAction.UPLOAD_DOCUMENT)
                await client.send_document(chat_id, file_name, caption="Translated Summary", reply_to_message_id=orig_msg_id)
                os.remove(file_name)
        else:
        ...
        await client.send_message(chat_id, f"❌ Error during summary and translation: {e}", reply_to_message_id=orig_msg_id)
