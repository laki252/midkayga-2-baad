...
GEMINI_API_KEY = "AIzaSyAKrnVxMMPIqSzovoUggXy5CQ_4HiI_NU"
...
# New constant for the welcome message and initial prompt
...
"""
LANG_PROMPT = "**Choose your file language for transcription using the below buttons:**"


def build_language_keyboard():
    buttons, row = [], []
    for i, (label, code) in enumerate(LANGS, 1):
        # Callback data format: lang|CODE|LABEL
...
async def start(client, message: Message):
    ...
    # The message we will edit after selection
    await message.reply_text(
        LANG_PROMPT,
        reply_markup=keyboard
    )

# New handler for /lang command
...
async def language_callback_query(client, callback_query):
    ...
    # Check if the message being edited is the /start prompt (to send the welcome message)
    is_start_prompt = callback_query.message.text.strip() == LANG_PROMPT.strip()

    if is_start_prompt:
        # Edit the language selection message to the welcome message (only on /start)
        await callback_query.message.edit_text(
            WELCOME_MESSAGE,
            reply_markup=None  # Remove the keyboard
        )
    else:
        # For /lang command, just edit to a confirmation message or delete/keep it simple.
        # We will just remove the keyboard and leave a subtle confirmation.
        await callback_query.message.edit_reply_markup(reply_markup=None)
        await callback_query.message.edit_text(f"Language set to: **{label}**")
        
    # Send an alert to the user (only callback, no direct chat message)
...
async def mode_callback_query(client, callback_query):
    ...
    await callback_query.message.edit_text(
        f"Output mode set to: **{mode_name}**",
        reply_markup=None  # Remove the keyboard
    )
...
async def handle_text(client, message: Message):
    ...
    # This part remains for any text input that isn't a command
...
async def clean_up_callback(client, callback_query):
    ...
    # Send status message *replying* to the original message
    status_msg = await client.send_message(chat_id, "🔄 Processing clean transcript...", reply_to_message_id=msg_id)
    ...
    try:
        ...
        await status_msg.delete() # Delete the processing status message
        
        if len(cleaned_text) > 4000:
            if mode == "💬 Split messages":
                for part in [cleaned_text[i:i+4000] for i in range(0, len(cleaned_text), 4000)]:
                    # Reply to the original transcription message
...
async def get_key_points_callback(client, callback_query):
    ...
    try:
        ...
        if len(summary) > 4000:
            ...
            # Reply to the original transcription message
            await client.send_message(chat_id, summary, reply_to_message_id=msg_id)
