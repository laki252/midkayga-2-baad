...
async def handle_media(client, message: Message):
    ...
    status_msg = await message.reply_text("⏳ **1/3** I'm downloading the file...", reply_to_message_id=message.id)
    ...
    try: file_path = await download_media(message)
        ...
        return
    await status_msg.edit_text("🎤 **2/3** I'm running the transcription...")
    ...
    try:
        ...
        if os.path.exists(file_path): os.remove(file_path)
    await status_msg.edit_text("✅ **3/3** I have finished the task, I am sending the result...")
