...
async def delete_message_after(client, chat_id, message_id, delay):
...
async def handle_text(client, message: Message):
    ...
    if text in LABEL_TO_CODE:
        ...
        user_lang[uid] = code
        msg = await message.reply_text(f"Language set to: **{code}**", parse_mode="markdown")
        try:
            asyncio.get_event_loop().create_task(delete_message_after(client, msg.chat.id, msg.id, 3))
        except:
            pass
        return
    if text in ["💬 Split messages", "📄 Text File"]:
        user_mode[uid] = text
        await message.reply_text(f"Output mode set to: **{text}**", parse_mode="markdown")
        return
