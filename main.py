...
MONGO_URI = "mongodb+srv://lakicalinuur_db_user:fLPXoE1SVw1jEwyW@cluster0.jja0fem.mongodb.net/?appName=Cluster0"
DB_NAME = "STTBot"
...
def get_user_data(user_id):
    if not users_col:
        return {"lang": None, "mode": "📄 Text File", "usage_count": 0}
    user = users_col.find_one({"_id": user_id})
    if not user:
        return {"lang": None, "mode": "📄 Text File", "usage_count": 0}
    return {
        "lang": user.get("lang"),
        "mode": user.get("mode", "📄 Text File"),
        "usage_count": user.get("usage_count", 0)
    }

def update_user_data(user_id, data_to_set):
    if not users_col:
        return
    users_col.update_one({"_id": user_id}, {"$set": data_to_set}, upsert=True)

def save_transcription(chat_id, message_id, text, origin_msg_id):
    if not trans_col:
        return
    doc_id = f"{chat_id}_{message_id}"
    trans_col.update_one(
        {"_id": doc_id},
        {"$set": {"text": text, "origin": origin_msg_id, "clean_usage": 0, "summarize_usage": 0}},
        upsert=True
    )

def get_transcription(chat_id, message_id):
    if not trans_col:
        return None
    doc_id = f"{chat_id}_{message_id}"
    return trans_col.find_one({"_id": doc_id})

def increment_action_usage(chat_id, message_id, action_type):
    if not trans_col:
        return None
    doc_id = f"{chat_id}_{message_id}"
    field_to_inc = f"{action_type}_usage"
    result = trans_col.find_one_and_update(
        {"_id": doc_id},
        {"$inc": {field_to_inc: 1}},
    ...
    if not result:
        return None
    return result.get(field_to_inc, 0)
...
async def ensure_joined(client, obj) -> bool:
    ...
    if count < 3:
        update_user_data(uid, {"usage_count": count + 1})
        return True
    try:
...
async def language_callback_query(client, callback_query: CallbackQuery):
    ...
    update_user_data(uid, {"lang": code})
...
async def mode_callback_query(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        return
    uid = callback_query.from_user.id
    update_user_data(uid, {"mode": mode_name})
...
async def handle_text(client, message: Message):
    ...
    if text in ["💬 Split messages", "📄 Text File"]:
        update_user_data(uid, {"mode": text})
...
async def handle_media(client, message: Message):
    ...
    if sent_message:
        try:
            ...
            save_transcription(sent_message.chat.id, sent_message.id, text, reply_msg_id)
            await sent_message.edit_reply_markup(keyboard)
        except Exception as e:
            logging.error(f"Failed to attach keyboard or save transcription: {e}")
...
async def clean_up_callback(client, callback_query: CallbackQuery):
    ...
    stored = get_transcription(chat_id, msg_id)
    if not stored:
        await callback_query.answer("Clean up unavailable (maybe expired or not found).", show_alert=True)
        return
        
    usage = stored.get("clean_usage", 0)
    if usage >= 1:
        await callback_query.answer("Clean up already used or unavailable.", show_alert=True)
        return

    increment_action_usage(chat_id, msg_id, "clean")
    
    stored_text = stored.get("text")
    orig_msg_id = stored.get("origin")
    
    if not stored_text or not orig_msg_id:
        await callback_query.answer("Original text not found.", show_alert=True)
        return
    ...
    try:
        ...
        uid = callback_query.from_user.id
        user_data = get_user_data(uid)
        lang = user_data.get("lang") or "en"
        mode = user_data.get("mode", "📄 Text File")
...
async def get_key_points_callback(client, callback_query: CallbackQuery):
    ...
    usage = stored.get("summarize_usage", 0)
    if usage >= 1:
        await callback_query.answer("Summarize already used or unavailable.", show_alert=True)
        return

    increment_action_usage(chat_id, msg_id, "summarize")
...
if __name__ == "__main__":
    if not db:
        logging.critical("MongoDB not connected. Bot cannot start.")
    else:
