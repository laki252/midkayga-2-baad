...
async def is_member(client, user_id):
    if not REQUIRED_CHANNEL:
        return True
    try:
        chat = await client.get_chat(REQUIRED_CHANNEL)
        chat_id = chat.id
    except Exception:
        try:
            chat_id = int(REQUIRED_CHANNEL)
        except Exception:
            return False
    try:
...
async def require_membership_for_message(client, message: Message):
    ...
    if await is_member(client, user_id):
        return True
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("➡️ Join channel", url=f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}")]])
    try:
        await message.reply_text("⚠️ To use this bot, please join the official channel first.", reply_markup=kb)
    except Exception:
        pass
    return False
...
async def require_membership_for_callback(client, callback_query: CallbackQuery):
    ...
    if await is_member(client, user_id):
        return True
    await callback_query.answer("Please join the channel to use the bot.", show_alert=True)
    try:
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("➡️ Join channel", url=f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}")]])
        await callback_query.message.reply_text("⚠️ To use this bot, please join the official channel first.", reply_markup=kb)
    except Exception:
        pass
    return False
