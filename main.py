...
def telegram_webhook():
    ...
    if not update:
        return {"ok": False}, 400
    try:
        if PYROGRAM_LOOP is None:
            return {"ok": False, "error": "client not ready"}, 503
        asyncio.run_coroutine_threadsafe(pyro_client.process_update(update), PYROGRAM_LOOP)
    except Exception as e:
        logging.exception("Failed to schedule update")
...
pyro_client = Client("media_transcriber", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
PYROGRAM_LOOP = None
...
@pyro_client.on_message(filters.command("help") & filters.private)
async def help_command(client, message: Message):
    if not await ensure_joined(client, message):
        return
    await message.reply_text(HELP_MESSAGE)

@pyro_client.on_message(filters.command("lang") & filters.private)
...
@pyro_client.on_callback_query(filters.regex(r"^lang\|"))
...
@pyro_client.on_message(filters.command("mode") & filters.private)
...
@pyro_client.on_callback_query(filters.regex(r"^mode\|"))
...
@pyro_client.on_message(filters.private & (filters.audio | filters.voice | filters.video | filters.document))
...
@pyro_client.on_callback_query(filters.regex(r"^clean\|"))
...
@pyro_client.on_callback_query(filters.regex(r"^summarize\|"))
...
if __name__ == "__main__":
    try:
        pyro_client.start()
        PYROGRAM_LOOP = getattr(pyro_client, "loop", None) or asyncio.get_event_loop()
        webhook_base = os.environ.get("WEBHOOK_URL")
        port = int(os.environ.get("PORT", 8080))
        if webhook_base:
            set_url = f"https://api.telegram.org/bot{BOT_TOKEN}/setWebhook"
            try:
                resp = requests.post(set_url, data={"url": f"{webhook_base.rstrip('/')}/webhook"})
                logging.info("setWebhook response: %s", resp.text)
            except Exception:
