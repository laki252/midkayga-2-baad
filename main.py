...
async def record_transcription(user_id, origin_message_id, text, language):
    await TRANSCRIPTIONS_COLLECTION.insert_one({
        "user_id": user_id,
...
async def get_bot_statistics():
    ...
    total_transcriptions = await TRANSCRIPTIONS_COLLECTION.count_documents({})
    ...
    most_used_langs_cursor = TRANSCRIPTIONS_COLLECTION.aggregate(pipeline)
    most_used_langs = await most_used_langs_cursor.to_list(length=5)
    
    most_langs_text = ""
    for item in most_used_langs:
        most_langs_text += f"• {item['_id']}: {item['count']} times\n"
    if not most_langs_text:
        most_langs_text = "No transcriptions recorded yet."
...
async def ensure_joined(client, obj) -> bool:
    ...
    if count < 3:
        ...
        try:
            await register_user(uid, obj.from_user.username, obj.from_user.first_name)
        except Exception:
            pass
        return True
    try:
...
async def language_callback_query(client, callback_query: CallbackQuery):
    ...
    user_lang[uid] = code
    try:
        await register_user(uid, callback_query.from_user.username, callback_query.from_user.first_name)
    except Exception:
        pass
    if origin == "start":
...
async def handle_media(client, message: Message):
    ...
    uid = message.from_user.id
    try:
        await register_user(uid, message.from_user.username, message.from_user.first_name)
    except Exception:
        pass
    ...
    if not text or text.startswith("Error:"):
        ...
        return
    try:
        await record_transcription(uid, message.id, text, lang)
    except Exception:
        pass
...
async def clean_up_callback(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        if not cleaned_text:
            ...
            return
        try:
            await record_transcription(uid, orig_msg_id, cleaned_text, lang)
        except Exception:
            pass
...
async def get_key_points_callback(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        if not summary:
            ...
            return
        try:
            await record_transcription(uid, orig_msg_id, summary, lang)
        except Exception:
            pass
        if len(summary) > 4095:
            if mode == "💬 Split messages":
