...
MONGO_URI = os.environ.get("MONGO_URI", f"mongodb+srv://{DB_USER}:{DB_PASSWORD}@cluster0.n4hdlxk.mongodb.net/?retryWrites=true&w=majority&appName={DB_APPNAME}")
...
DEFAULT_ASSEMBLY_KEYS = os.environ.get("DEFAULT_ASSEMBLY_KEYS", "e27f99e6c34e44a4af5e0934b34b3e6f,a6d887c307044ee4a918b868a770e8ef,0272c2f92b1e4b1a96fcec55975c5c2e,b77044ed989546c9ab3a064df4a46d8c,2b7533db7ec849668716b00cb64a9235,defa21f626764d71a1373437f6300d80,26293b7d8dbf43d883ce8a43d3c06f63")
DEFAULT_GEMINI_KEYS = os.environ.get("DEFAULT_GEMINI_KEYS", "AIzaSyADfan-yL9WdrlVd3vzbCdJM7tXbA72dG,AIzaSyAKrnVxMMPIqSzovoUggXy5CQ_4Hi7I_NU,AIzaSyD0sYw4zzlXhbSV3HLY9wM4zCqX8ytR8zQ")
...
media_app = Client("media_transcriber", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
@media_app.on_message(filters.command("start") & filters.private)
...
@media_app.on_message(filters.command("help") & filters.private)
async def help_command(client, message: Message):
    if not await ensure_joined(client, message):
        return
    await message.reply_text(HELP_MESSAGE)

@media_app.on_message(filters.command("lang") & filters.private)
...
@media_app.on_callback_query(filters.regex(r"^lang\|"))
...
@media_app.on_message(filters.command("mode") & filters.private)
...
@media_app.on_callback_query(filters.regex(r"^mode\|"))
...
@media_app.on_message(filters.private & filters.text)
...
@media_app.on_message(filters.private & (filters.audio | filters.voice | filters.video | filters.document))
...
@media_app.on_callback_query(filters.regex(r"^clean\|"))
...
@media_app.on_callback_query(filters.regex(r"^summarize\|"))
...
TELEGRAM_TOKEN = os.environ.get("SPAM_BOT_TOKEN", "8581764529:AAGShqZAxGe2pRJOxEqB6HDLxOMOSGOgrxs")
WEBHOOK_URL = os.environ.get("WEBHOOK_URL", "https://midkayga-2-baad-1ggd.onrender.com")

spam_bot = telebot.TeleBot(TELEGRAM_TOKEN)

@spam_bot.message_handler(func=lambda m: m.chat.type in ["group", "supergroup"] and m.content_type == 'text')
def anti_spam_filter(message):
    try:
        bot_member = spam_bot.get_chat_member(message.chat.id, spam_bot.get_me().id)
        if bot_member.status not in ['administrator', 'creator']:
            return
        user_member = spam_bot.get_chat_member(message.chat.id, message.from_user.id)
        if user_member.status in ['administrator', 'creator']:
            return
        text = message.text or ""
        if (len(text) > 120 or re.search(r"https?://", text) or "t.me/" in text or re.search(r"@\w+", text)):
            try:
                spam_bot.delete_message(chat_id=message.chat.id, message_id=message.message_id)
            except Exception:
                pass
    except Exception as e:
        logging.warning(f"Anti-spam check failed: {e}")

def set_spam_bot_info():
    cmds = [
        telebot.types.BotCommand("start", "Start the bot"),
        telebot.types.BotCommand("help", "Show help message")
    ]
    try:
        spam_bot.set_my_commands(cmds)
        spam_bot.set_my_description("To keep your Telegram groups clean, focused, and free from spam or excessively long messages that disrupt conversations, use this bot.")
    except Exception:
        pass

@spam_bot.message_handler(commands=['start'])
def handle_start(message):
    try:
        spam_bot.send_message(message.chat.id, "👋 Welcome! Add me to your group and make me an admin to remove spam.")
    except Exception:
        pass

@spam_bot.message_handler(commands=['help'])
def handle_help(message):
    try:
        spam_bot.send_message(message.chat.id, "Commands:\n/start - Start bot\n/help - Show help message\n\nThis bot only removes spam from groups when it is an admin.", parse_mode="Markdown")
    except Exception:
        pass

@flask_app.route('/telebot', methods=['POST'])
def telebot_webhook():
    if request.headers.get('content-type') == 'application/json':
        update = telebot.types.Update.de_json(request.data.decode('utf-8'))
        try:
            spam_bot.process_new_updates([update])
        except Exception:
            pass
        return '', 200
    else:
...
@flask_app.route('/telebot/set_webhook', methods=['GET'])
def set_wh():
    try:
        spam_bot.set_webhook(url=WEBHOOK_URL)
        return f"Webhook set to {WEBHOOK_URL}"
    except Exception as e:
        return f"Failed to set webhook: {e}", 500

@flask_app.route('/telebot/delete_webhook', methods=['GET'])
def del_wh():
    try:
        spam_bot.delete_webhook()
...
if __name__ == "__main__":
    threading.Thread(target=start_media_app, daemon=True).start()
    set_spam_bot_info()
    try:
        spam_bot.set_webhook(url=WEBHOOK_URL)
    except Exception:
