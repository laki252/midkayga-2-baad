        ...
        try:
            ...
            if transcript.error:
                ...
                if "authentication" in error_msg or "authorization" in error_msg or "not authorized" in error_msg or "invalid api key" in error_msg:
                    logging.warning(f"AssemblyAI key at index {key_index} failed: {transcript.error}")
                    continue
                else:
            ...
            logging.warning(f"AssemblyAI key at index {key_index} failed with exception: {e}")

    raise RuntimeError(f"All {num_keys} AssemblyAI API keys failed.")
