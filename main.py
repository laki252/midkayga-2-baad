...
# 1. Configuration (Ma jiro wax la beddelay)
...
WEBHOOK_BASE = os.environ.get("WEBHOOK_BASE", "https://midkayga-2-baad-y8r7.onrender.com")
...
# 2. Functions for User Preferences (Ma jiro wax la beddelay)
...
def build_action_keyboard(chat_id, message_id, text_length):
    buttons = []
    # Haddii aad doonayso in aad dib u celiso 'Clean transcript', fur khadka hoose
    ...
    # Haddii aanay jirin wax badhan ah, wuxuu celinayaa InlineKeyboardMarkup oo madhan
    if not buttons:
        return None
...
# 3. Message Handlers (Waxa lagu daray 'try...except' meesha ay dhibaato ka imaan karto)
...
async def lang_command(client, message: Message):
    ...
    try:
        ...
        logging.error(f"Error sending lang command message: {e}")
...
async def language_callback_query(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        logging.warning(f"Error editing/deleting message after lang selection: {e}")
...
async def choose_mode(client, message: Message):
    ...
    try:
        ...
        logging.error(f"Error sending mode selection message: {e}")
...
# 4. Qaybta Khaladku Ka Dhacayay (Waxaan ku darnay xakamayn khaladaad oo muhiim ah)
...
async def handle_media(client, message: Message):
    ...
    try:
        ...
        # Haddii fariinta diristeeda ay ku timaado khalad
        logging.error(f"Failed to send transcription message: {e}")
        return # Jooji haddii aysan farriin dirmin
        
    # ⚠️ Qaybta Khaladku Ka Dhacayay (Inline Keyboard-ka)
    if sent_message:
        try:
            keyboard = build_action_keyboard(sent_message.chat.id, sent_message.id, len(text))
            # Haddi aanay jirin wax badhan ah, ma hagaajinayso (edit) fariinta
            if keyboard:
            ...
            # Tani waa xakamayntii khaladaadka REPLY_MARKUP_INVALID
            logging.error(f"Failed to attach keyboard or init usage: {e}")
            # Botku wuu sii soconayaa xitaa haddii keyboard-ku guuldaraysto
            # Waxaad ku dari kartaa: await client.send_message(message.chat.id, "Keyboard attachment failed, but text was sent.", reply_to_message_id=sent_message.id)

# 5. Callback Query Handlers (Waxaan ku darnay xakamayn khaladaad oo u dhigan)
@app.on_callback_query(filters.regex(r"^clean\|"))
async def clean_up_callback(client, callback_query: CallbackQuery):
    if not await ensure_joined(client, callback_query):
        return
    # Xakamaynta Khaladka ee go'aan gaadhista
    try:
    ...
    # Xakamaynta Khaladka ee jawaab celinta (answer)
    try:
        await callback_query.answer("Cleaning up...", show_alert=False)
    except Exception:
        pass # Waa caadi haddii answer-ku guuldarraysto
    ...
    try:
        ...
        mode = get_user_mode(uid, "📄 Text File")
        instruction = f"Clean and normalize this text  Send me a clean, written text in {lang}), even if it is currently written in another language Do not add introductions or explanations."
        ...
        # Xakamaynta Khaladka ee dirista fariinta
            ...
            if mode == "💬 Split messages":
                for part in [cleaned_text[i:i+MAX_MESSAGE_CHUNK] for i in range(0, len(cleaned_text), MAX_MESSAGE_CHUNK)]:
                    try:
                        ...
                        logging.error(f"Error sending split message: {e}")
            else:
        ...
        try:
            ...
            pass # Iskaga gudub haddii dirista khaladku ay guuldarrayso
...
# 6. Telebot/Flask/Startup (Ma jiro wax la beddelay oo muhiim ah)
