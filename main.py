...
def upload_file_to_assemblyai_local(api_key, file_path, timeout=REQUEST_TIMEOUT_ASSEMBLYAI):
    with open(file_path, 'rb') as f:
        ...
        resp.raise_for_status()
        result = resp.json()
        return result.get("upload_url") or result.get("url")

def transcribe_file_with_assemblyai_local(file_path: str, lang_code: str = "en"):
    if not ASSEMBLYAI_API_KEYS:
        raise RuntimeError("ASSEMBLYAI_API_KEYS not set")
    last_exception = None
    for api_key in ASSEMBLYAI_API_KEYS:
        try:
            upload_url = upload_file_to_assemblyai_local(api_key, file_path)
            if not upload_url:
            ...
            submit_resp = requests.post(submit_url, headers=headers, json=config, timeout=REQUEST_TIMEOUT_ASSEMBLYAI)
            ...
            while True:
                poll_resp = requests.get(poll_url, headers={"Authorization": api_key}, timeout=REQUEST_TIMEOUT_ASSEMBLYAI)
...
def transcribe_file(file_path: str, lang_code: str = "en") -> str:
    return transcribe_file_with_assemblyai_local(file_path, lang_code)
...
async def handle_media(client, message: Message):
    ...
    if file_size and file_size > TELEGRAM_MAX_BYTES:
        ...
        await message.reply_text(f"Just send me a file less than {max_mb}MB 😎", reply_to_message_id=message.id)
        return
    file_path = os.path.join(DOWNLOADS_DIR, f"{message.id}{ext or ''}")
    try:
        await download_media(message, file_path)
    except Exception as e:
        await message.reply_text(f"⚠️ Download error: {e}", reply_to_message_id=message.id)
        return
    await client.send_chat_action(message.chat.id, ChatAction.TYPING)
    try:
        ...
        await message.reply_text(f"❌ Transcription error: {e}", reply_to_message_id=message.id)
