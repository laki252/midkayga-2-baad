...
async def language_callback_query(client, callback_query):
    if callback_query.message.chat.type == "private" and not check_subscription(callback_query.from_user.id):
        await send_subscription_message(callback_query.message.chat.id)
        await callback_query.answer("You must join the required channel first.", show_alert=True)
        return
    try:
...
async def mode_callback_query(client, callback_query):
    ...
    await callback_query.message.edit_text(f"Mode set to: {mode_name}", reply_markup=None)
...
async def handle_text(client, message: Message):
    ...
    if text in ["💬 Split messages", "📄 Text File"]:
        user_mode[uid] = text
        await message.reply_text(f"Mode set to: {text}")
        return
