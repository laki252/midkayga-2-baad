...
@app.on_message(filters.private & filters.text & ~filters.command) # Looma baahna ~filters.command, laakiin waa lagu daray si loo hubiyo
async def handle_text(client, message: Message):
    ...
    # Haddi aanu ahayn command, luqad, ama mode, soo bandhig language keyboard-ka.
    if text.startswith('/'): # Waxaa laga yaabaa inuu yahay command aan la garanayn
         await message.reply_text("I'm sorry, that command is not recognized. Please use a media file, or choose an option from the keyboard.")
    
    keyboard = get_language_keyboard()
    await message.reply_text("⚠️ **Unknown text.** Please choose your **language** or send a media file:", reply_markup=keyboard)
