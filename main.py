...
async def choose_mode(client, message: Message):
    buttons = [
        [InlineKeyboardButton("💬 Split messages", callback_data="mode|split")],
        [InlineKeyboardButton("📄 Text File", callback_data="mode|file")]
    ]
    keyboard = InlineKeyboardMarkup(buttons)
...
async def handle_media(client, message: Message):
    ...
    if len(text) > 4000:
        if mode == "💬 Split messages":
            ...
            btn_msg = await client.send_message(message.chat.id, "Transcription file uploaded. Use buttons below.", reply_to_message_id=reply_msg_id)
            temp_transcriptions[(btn_msg.chat.id, btn_msg.id)] = text
            keyboard = attach_action_buttons(text, btn_msg.chat.id, btn_msg.id)
            await client.edit_message_reply_markup(btn_msg.chat.id, btn_msg.id, reply_markup=keyboard)
            os.remove(file_name)
    else:
        await client.send_chat_action(message.chat.id, ChatAction.TYPING)
...
async def handle_cleanup_callback(client, call: CallbackQuery):
    ...
    if len(cleaned_text) > 4000:
        if mode == "💬 Split messages":
            ...
            btn_msg = await client.send_message(chat_id, "Cleaned file uploaded. Use buttons below.", reply_to_message_id=msg_id)
            temp_transcriptions[(btn_msg.chat.id, btn_msg.id)] = cleaned_text
            keyboard = attach_action_buttons(cleaned_text, btn_msg.chat.id, btn_msg.id)
            await client.edit_message_reply_markup(btn_msg.chat.id, btn_msg.id, reply_markup=keyboard)
            os.remove(file_name)
    else:
        await client.send_chat_action(chat_id, ChatAction.TYPING)
...
async def handle_summarize_callback(client, call: CallbackQuery):
    ...
    if len(summary_text) > 4000:
        if mode == "💬 Split messages":
            ...
            btn_msg = await client.send_message(chat_id, "Summary file uploaded. Use buttons below.", reply_to_message_id=msg_id)
            temp_transcriptions[(btn_msg.chat.id, btn_msg.id)] = summary_text
            keyboard = attach_action_buttons(summary_text, btn_msg.chat.id, btn_msg.id)
            await client.edit_message_reply_markup(btn_msg.chat.id, btn_msg.id, reply_markup=keyboard)
            os.remove(file_name)
    else:
        await client.send_chat_action(chat_id, ChatAction.TYPING)
...
async def handle_lang_callback(client, call: CallbackQuery):
    await call.answer()
    ...
    await client.edit_message_text(chat_id, msg_id, f"Language set to: **{code}**")
    await client.edit_message_reply_markup(chat_id, msg_id, reply_markup=None)

@app.on_callback_query(filters.regex(r"^mode\|"))
async def handle_mode_callback(client, call: CallbackQuery):
    await call.answer()
    _, mode_code = call.data.split("|", 1)
    uid = call.from_user.id
    display = "💬 Split messages" if mode_code == "split" else "📄 Text File"
    user_mode[uid] = display
    chat_id = call.message.chat.id
    msg_id = call.message.id
    await client.edit_message_text(chat_id, msg_id, f"Output mode set to: **{display}**")
    await client.edit_message_reply_markup(chat_id, msg_id, reply_markup=None)
