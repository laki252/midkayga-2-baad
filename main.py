...
def keep_alive():
    try:
        ...
        if "mobile" in ua_lower or "android" in ua_lower or "iphone" in ua_lower:
            ...
            device = "Desktop"
        elif "bot" in ua_lower or "curl" in ua_lower or "wget" in ua_lower:
            device = "Bot/Script"
        entry = {"time": now, "ip": ip, "remote": remote, "ua": ua, "device": device, "method": method, "path": path}
        with VISITOR_LOCK:
            VISITOR_LOG.insert(0, entry)
            if len(VISITOR_LOG) > MAX_VISITORS_STORED:
                VISITOR_LOG.pop()
        ...
        total_visits = len(VISITOR_LOG)
        rows = []
        with VISITOR_LOCK:
            for v in VISITOR_LOG[:50]:
                t = html.escape(v["time"])
                ip_html = html.escape(v["ip"])
                remote_html = html.escape(v["remote"])
                ua_html = html.escape(v["ua"])
                device_html = html.escape(v["device"])
                method_html = html.escape(v["method"])
                path_html = html.escape(v["path"])
                rows.append(f"<tr><td>{t}</td><td>{ip_html}</td><td>{remote_html}</td><td>{device_html}</td><td>{method_html}</td><td style='max-width:400px;word-break:break-word'>{ua_html}</td><td>{path_html}</td></tr>")
        rows_html = "\n".join(rows)
...
<style>
body{{font-family:Arial,Helvetica,sans-serif;margin:20px;background:#f7f7f7;color:#111}}
.container{{max-width:1100px;margin:0 auto;background:#fff;padding:20px;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,0.08)}}
h1{{margin-top:0}}
.stats{{display:flex;gap:20px;flex-wrap:wrap;margin-bottom:16px}}
.card{{padding:12px 16px;border-radius:6px;background:#f2f6fb;border:1px solid #e1e7f0}}
...
<body>
<div class="container">
<h1>Bot Live Status</h1>
<div class="stats">
<div class="card"><strong>Uptime</strong><div class="small">{html.escape(uptime_str)}</div></div>
<div class="card"><strong>Started at (UTC)</strong><div class="small">{html.escape(datetime.datetime.utcfromtimestamp(START_TIME).replace(microsecond=0).isoformat()+"Z")}</div></div>
<div class="card"><strong>Total visits stored</strong><div class="small">{total_visits}</div></div>
<div class="card"><strong>Showing</strong><div class="small">Last 50 visits</div></div>
</div>
<h2>Recent Visitors</h2>
<table>
<thead><tr><th>Time (UTC)</th><th>IP (X-Forwarded)</th><th>Remote Addr</th><th>Device</th><th>Method</th><th>User-Agent</th><th>Path</th></tr></thead>
<tbody>
{rows_html}
</tbody>
</table>
