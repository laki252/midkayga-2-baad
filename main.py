...
try:
    ...
    try:
        for doc in settings_col.find({"type": "lang"}):
            user_lang[int(doc["_id"])] = doc["value"]
    except Exception:
        pass
    try:
        for doc in settings_col.find({"type": "mode"}):
            user_mode[int(doc["_id"])] = doc["value"]
    except Exception:
        pass
    logging.info("Connected to MongoDB")
except Exception as e:
    logging.warning("MongoDB not available or connection failed: %s", e)
    settings_col = None

def save_user_lang_db(uid: int, code: str):
    user_lang[uid] = code
    if settings_col:
        try:
            settings_col.update_one({"_id": uid, "type": "lang"}, {"$set": {"value": code}}, upsert=True)
        except Exception as e:
            logging.warning("Failed to save user_lang to DB: %s", e)

def save_user_mode_db(uid: int, mode_name: str):
    user_mode[uid] = mode_name
    if settings_col:
        try:
            settings_col.update_one({"_id": uid, "type": "mode"}, {"$set": {"value": mode_name}}, upsert=True)
        except Exception as e:
            logging.warning("Failed to save user_mode to DB: %s", e)
...
async def mode_callback_query(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        return
    uid = callback_query.from_user.id
    save_user_mode_db(uid, mode_name)
...
async def handle_media(client, message: Message):
    ...
    if len(text) > MAX_MESSAGE_LEN:
        if mode == "💬 Split messages":
            for part in [text[i:i+MAX_MESSAGE_LEN] for i in range(0, len(text), MAX_MESSAGE_LEN)]:
                await client.send_chat_action(message.chat.id, ChatAction.TYPING)
...
async def clean_up_callback(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        if not cleaned_text:
            ...
            return
        if len(cleaned_text) > MAX_MESSAGE_LEN:
            if mode == "💬 Split messages":
                for part in [cleaned_text[i:i+MAX_MESSAGE_LEN] for i in range(0, len(cleaned_text), MAX_MESSAGE_LEN)]:
...
async def get_key_points_callback(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        if not summary:
            ...
            return
        if len(summary) > MAX_MESSAGE_LEN:
            if mode == "💬 Split messages":
                for part in [summary[i:i+MAX_MESSAGE_LEN] for i in range(0, len(summary), MAX_MESSAGE_LEN)]:
                    await client.send_message(chat_id, part, reply_to_message_id=orig_msg_id)
            else:
