...
async def ensure_joined(client, obj) -> bool:
    if isinstance(obj, CallbackQuery):
        ...
        uid = obj.from_user.id
        reply_target = obj
    usage = user_usage_count.get(uid, 0)
    if usage < 3:
        return True
    try:
        ...
        is_member = False
    if is_member:
        stored = user_channel_prompt.pop(uid, None)
        if stored:
            try:
                await client.edit_message_text(stored.get("chat_id"), stored.get("message_id"), "✅ Done ✅")
            except Exception:
                pass
        return True
    ...
    try:
        ...
        if isinstance(obj, CallbackQuery):
            try:
                ...
                pass
        try:
            sent = await reply_target.reply_text(text, reply_markup=kb)
        except Exception:
            sent = await client.send_message(uid, text, reply_markup=kb)
        if sent:
            user_channel_prompt[uid] = {"chat_id": sent.chat.id, "message_id": sent.id}
    except Exception:
        try:
            await client.send_message(uid, text, reply_markup=kb)
        except Exception:
...
async def handle_media(client, message: Message):
    ...
    prev_count = user_usage_count.get(uid, 0)
    user_usage_count[uid] = prev_count + 1
    if user_usage_count[uid] >= 3:
        if not await is_user_in_channel(client, uid):
            if uid not in user_channel_prompt:
                try:
                    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔗 Join Channel", url=f"https://t.me/{REQUIRED_CHANNEL.strip('@')}")]])
                    join_text = f"🚫 Please join the channel {REQUIRED_CHANNEL} to continue using the bot"
                    sent = await client.send_message(uid, join_text, reply_markup=kb)
                    if sent:
                        user_channel_prompt[uid] = {"chat_id": sent.chat.id, "message_id": sent.id}
                except Exception:
                    pass
