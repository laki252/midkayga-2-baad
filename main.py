...
def transcribe_file(file_path: str, lang_code: str = "en") -> str:
    ...
    for i in range(num_keys):
        ...
        try:
            # Halkan ayaa lagu sameeyay hagaajinta: Waxaan isticmaalaynaa aai.Client ka hor Transcriber
            client = aai.Client(api_key=api_key) 
            transcriber = aai.Transcriber(client=client) # U dhiib Client-ka halkii api_key
            ...
            if transcript.error:
                ...
                # Baar Authentication/API Key errors
            ...
            # Baar Authentication/API Key errors oo ka imanaya Python SDK-ga
            error_message = str(e)
            if "Authentication" in error_message or "Invalid API Key" in error_message or "unexpected keyword argument 'api_key'" in error_message:
                 # Hadda waxaan u wareegaynaa furaha xiga haddii uu jiro khaladka 'api_key' oo lama filaan ah
                logging.warning(f"AssemblyAI key at index {key_index} failed with SDK exception (Auth/Init Error): {error_message}")
                continue 
