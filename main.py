...
DEFAULT_GEMINI_KEYS = "AIzaSyD0XqLRGRAfLBhUe2xrHHZSN-o8F-HKUHo"
...
pyro = Client("media_transcriber", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
def ask_gemini_with_genai(instruction, text, timeout=REQUEST_TIMEOUT_GEMINI):
...
def ask_gemini_rest(text, instruction, timeout=REQUEST_TIMEOUT_GEMINI):
...
def ask_gemini(text, instruction, timeout=REQUEST_TIMEOUT_GEMINI):
    try:
        if has_genai and genai_client is not None:
            return ask_gemini_with_genai(instruction, text, timeout=timeout)
    except Exception:
        pass
    return ask_gemini_rest(text, instruction, timeout=timeout)
...
@pyro.on_message(filters.command("lang") & filters.private)
...
@pyro.on_callback_query(filters.regex(r"^lang\|"))
...
@pyro.on_message(filters.command("mode") & filters.private)
...
@pyro.on_callback_query(filters.regex(r"^mode\|"))
...
@pyro.on_message(filters.private & filters.text)
...
@pyro.on_message(filters.private & (filters.audio | filters.voice | filters.video | filters.document))
...
@pyro.on_callback_query(filters.regex(r"^clean\|"))
...
@pyro.on_callback_query(filters.regex(r"^summarize\|"))
...
tele_token = os.environ.get("WEBHOOK_BOT_TOKEN", BOT_TOKEN)
telebot_bot = telebot.TeleBot(tele_token, threaded=False)

@web_app.route("/", methods=["GET", "POST", "HEAD"])
def keep_alive():
    if request.method in ("GET", "HEAD"):
        return "Bot is alive ✅", 200
    return "", 200

@web_app.route("/tg_webhook", methods=["POST"])
def tg_webhook():
...
@web_app.route("/set_webhook", methods=["GET", "POST"])
...
@web_app.route("/delete_webhook", methods=["GET", "POST"])
...
def set_webhook_on_startup():
    try:
        if not WEBHOOK_URL:
            logging.info("WEBHOOK_URL not provided, skipping webhook set")
            return
        telebot_bot.delete_webhook()
        time.sleep(1)
        telebot_bot.set_webhook(url=WEBHOOK_URL)
...
@telebot_bot.message_handler(content_types=["text", "photo", "audio", "voice", "video", "sticker", "document", "animation"])
def handle_all_messages(message):
    MEDIA_TO_TEXT_BOT = os.environ.get("MEDIA_TO_TEXT_BOT", "https://t.me/MediaToTextBot")
    reply = (
        f"[Use our new bot]({MEDIA_TO_TEXT_BOT})\n"
        f"[استخدم بوتنا الجديد]({MEDIA_TO_TEXT_BOT}\n)"
        f"[👇🏻👇🏻👇🏻👇🏻👇🏻👇🏻👇🏻]({MEDIA_TO_TEXT_BOT})"
    )
    try:
...
def run_flask():
    ...
    web_app.run(host="0.0.0.0", port=flask_port)
