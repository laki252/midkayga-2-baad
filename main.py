...
async def send_transcript_output(client, chat_id, reply_msg_id, text, mode, original_msg_id):
    with memory_lock:
        user_transcriptions.setdefault(chat_id, {})[reply_msg_id] = text
        action_usage[f"{chat_id}|{reply_msg_id}|clean_up"] = 0
        action_usage[f"{chat_id}|{reply_msg_id}|get_key_points"] = 0
    ...
    if len(text) > 4000:
        if mode == "💬 Split messages":
            ...
            if last_sent:
                await last_sent.edit_reply_markup(keyboard)
        else:
            ...
            await client.send_document(chat_id, file_name, caption="Open this file and copy the text inside 👍", reply_to_message_id=original_msg_id, reply_markup=keyboard)
            os.remove(file_name)
    else:
        await client.send_chat_action(chat_id, ChatAction.TYPING)
        await client.send_message(chat_id, text, reply_to_message_id=original_msg_id, reply_markup=keyboard)
...
async def get_key_points_callback(client, callback_query):
    ...
    await status_msg.edit_text(summary or "No Summary returned.", reply_markup=None)
...
async def clean_up_callback(client, callback_query):
    ...
    try:
        ...
        await status_msg.delete()
        reply_to_id = callback_query.message.reply_to_message.id if callback_query.message.reply_to_message else callback_query.message.id
