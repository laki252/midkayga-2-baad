...
async def mode_callback_query(client, callback_query):
    ...
    await callback_query.message.edit_text(f"Output mode set to: **{mode_name}**", reply_markup=None)
...
async def handle_media(client, message: Message):
    ...
    if sent_message:
        try:
            ...
            user_transcriptions.setdefault(sent_message.chat.id, {})[sent_message.id] = {"text": text, "orig_id": message.id}
...
async def clean_up_callback(client, callback_query):
    ...
    try:
        ...
        orig_msg_id = stored.get("orig_id")
