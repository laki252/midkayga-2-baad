...
# --- Qaybta Logging iyo Flask (Waxba kama bedelin) ---
...
if not all([API_ID, API_HASH, BOT_TOKEN]):
    logging.error("CRITICAL: Missing API_ID, API_HASH, or BOT_TOKEN environment variables.")
    exit(1)

try:
    API_ID = int(API_ID)
except ValueError:
    logging.error("CRITICAL: API_ID must be an integer.")
    exit(1)

ASSEMBLYAI_API_KEY_STRING = os.environ.get("ASSEMBLYAI_API_KEYS") or ""
ASSEMBLYAI_API_KEYS = [key.strip() for key in ASSEMBLYAI_API_KEY_STRING.split(",") if key.strip()]
current_assemblyai_key_index = 0
assemblyai_key_lock = threading.Lock()

# Waxaan ku hagaajiyay inay si fiican u shaqeyso xitaa haddii aad meel bannaan ku darto furayaasha
GEMINI_API_KEY_STRING = os.environ.get("GEMINI_API_KEYS") or ""
GEMINI_API_KEYS = [key.strip() for key in GEMINI_API_KEY_STRING.split(",") if key.strip()]
...
def ask_gemini(text, instruction, timeout=REQUEST_TIMEOUT_GEMINI):
    ...
    for i in range(num_keys):
        ...
        try:
            ...
            logging.warning(f"Gemini key {key_index} gave valid response but no candidates: {json.dumps(result)}")

        except requests.exceptions.RequestException as e:
            logging.warning(f"Gemini API key at index {key_index} failed: {e}")
        except Exception as e:
            logging.error(f"Unexpected error with Gemini key {key_index}: {e}")
    
    raise RuntimeError(f"All {num_keys} Gemini API keys failed.")
...
def transcribe_file(file_path: str, lang_code: str = "en") -> str:
    global current_assemblyai_key_index
    if not ASSEMBLYAI_API_KEYS:
        raise RuntimeError("ASSEMBLYAI_API_KEY not set")

    num_keys = len(ASSEMBLYAI_API_KEYS)
    start_index = current_assemblyai_key_index
    
    for i in range(num_keys):
        with assemblyai_key_lock:
            key_index = (start_index + i) % num_keys
            api_key = ASSEMBLYAI_API_KEYS[key_index]
        ...
        try:
            ...
            if transcript.error:
                ...
                # Cilada asalka ah waxay ahayd in khalad kasta oo yimaada uu u wareegayo furaha kale
                # Hadda waxaan si gaar ah u hubineynaa khaladaadka Authentication
                if "authentication" in error_msg or "authorization" in error_msg or "not authorized" in error_msg or "invalid api key" in error_msg:
                    logging.warning(f"AssemblyAI key at index {key_index} failed due to auth: {transcript.error}")
                    continue  # U wareeg furaha xiga
                else:
                    # Haddii uu jiro khalad aan ahayn Auth (tusaale, file format xun), soo celi fariinta khaladka isla markiiba
                    return f"Error: AssemblyAI reported: {transcript.error}"

            with assemblyai_key_lock:
                current_assemblyai_key_index = key_index
            return transcript.text

        except Exception as e:
            # AssemblyAI SDK wuxuu tuuri karaa khaladaadka sidoo kale
            error_message = str(e)
            if "Authentication" in error_message or "Invalid API Key" in error_message:
                logging.warning(f"AssemblyAI key at index {key_index} failed with SDK exception (Auth): {error_message}")
                continue # U wareeg furaha xiga
            
            logging.warning(f"AssemblyAI key at index {key_index} failed with general exception: {e}")
            # Haddii uu jiro khalad guud oo aan ahayn Auth, jooji isku dayga oo soo celi
            return f"Error: SDK Exception occurred: {e}"

    # Haddii dhammaan isku dayadii ay ku fashilmeen khaladaad Authentication awgeed, tan soo celi
    raise RuntimeError(f"All {num_keys} AssemblyAI API keys failed due to potential authentication/rate limit issues.")
...
async def ensure_joined(client, obj) -> bool:
    ...
    text = f"🚫 You have used the bot {count} times. Please join the channel {REQUIRED_CHANNEL} to continue using it."
    try:
        if isinstance(obj, CallbackQuery):
            try:
                await obj.answer("🚫 Please join the channel to continue", show_alert=True)
            except Exception:
                pass
