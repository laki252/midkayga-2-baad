...
from flask import Flask, render_template_string, request # Added Flask imports
from datetime import datetime, timedelta

# --- Global Variables for Flask and Uptime ---
START_TIME = datetime.now()
VISIT_LOG = []
MAX_VISIT_LOG = 100 # Xaddiga log-ga booqashooyinka
# --- Global Variables for Flask and Uptime ---


# --- Configuration (unchanged mostly) ---
...
# *** Changed MAX_CHARS to 4095 as requested ***
MAX_CHARS = 4095
# *** Changed MAX_CHARS to 4095 as requested ***
...
def status():
    ...
    VISIT_LOG.insert(0, {"time": timestamp, "ip": ip_address, "device": user_agent})
    if len(VISIT_LOG) > MAX_VISIT_LOG:
        VISIT_LOG.pop() # Keep the log size manageable

    uptime = get_uptime()

    # HTML content to display the status
    html_content = f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            ...
            h1 {{ color: #007bff; }}
            h2 {{ border-bottom: 2px solid #eee; padding-bottom: 5px; margin-top: 20px; color: #555; }}
    ...
    <body>
        <div class="container">
            ...
            <h2>⏱️ Uptime</h2>
            <p style="font-size: 1.2em; font-weight: bold;">{uptime}</p>

            <h2>🌍 Recent Visitors ({len(VISIT_LOG)} logged)</h2>
            <table>
                <thead>
                    <tr>
                        <th>Time</th>
...
def run_flask():
    ...
    # Run on all interfaces, default port is 5000
...
# --- Bot Functions (largely unchanged, adjusted MAX_CHARS) ---
...
async def handle_media(client, message: Message):
    ...
    if len(text) > MAX_CHARS:
        if mode == "💬 Split messages":
            for part in [text[i:i+MAX_CHARS] for i in range(0, len(text), MAX_CHARS)]: # Used MAX_CHARS
                await client.send_chat_action(message.chat.id, ChatAction.TYPING)
...
async def clean_up_callback(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        if not cleaned_text:
            ...
            return
        if len(cleaned_text) > MAX_CHARS: # Used MAX_CHARS
            if mode == "💬 Split messages":
                for part in [cleaned_text[i:i+MAX_CHARS] for i in range(0, len(cleaned_text), MAX_CHARS)]: # Used MAX_CHARS
...
async def get_key_points_callback(client, callback_query: CallbackQuery):
    ...
    try:
        ...
        if not summary:
            ...
            return
        if len(summary) > MAX_CHARS: # Used MAX_CHARS
            if mode == "💬 Split messages":
                for part in [summary[i:i+MAX_CHARS] for i in range(0, len(summary), MAX_CHARS)]: # Used MAX_CHARS
                    await client.send_message(chat_id, part, reply_to_message_id=orig_msg_id)
            else:
