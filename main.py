...
DEFAULT_ASSEMBLYAI_KEYS = "91f15c103dbd4b859466a29ee849e3ef"
DEFAULT_GEMINI_KEYS = "AIzaSyDCOMrP8YYSr5t8N7WZoMLQnKOQR4ByTvo"

ASSEMBLYAI_API_KEYS = [t.strip() for t in os.environ.get("ASSEMBLYAI_API_KEYS", os.environ.get("ASSEMBLYAI_API_KEY", DEFAULT_ASSEMBLYAI_KEYS)).split(",") if t.strip()]
GEMINI_API_KEYS = [t.strip() for t in os.environ.get("GEMINI_API_KEYS", os.environ.get("GEMINI_API_KEY", DEFAULT_GEMINI_KEYS)).split(",") if t.strip()]
...
def ask_gemini(text, instruction, timeout=REQUEST_TIMEOUT_GEMINI):
    ...
    raise RuntimeError("All Gemini API keys failed or no keys provided.")
...
def transcribe_file(file_path: str, lang_code: str = "en") -> str:
    ...
    for api_key in ASSEMBLYAI_API_KEYS:
        try:
            ...
            logging.warning(f"AssemblyAI key failed: {e}")
            last_exception = e
            continue
            
    if last_exception:
        return f"Error: {str(last_exception)}"
    return "Error: All AssemblyAI keys failed or no keys provided."
...
async def handle_media(client, message: Message):
    ...
    if file_size and file_size > TELEGRAM_MAX_BYTES:
        max_display_gb = TELEGRAM_MAX_BYTES / (1024 * 1024 * 1024)
        await message.reply_text(f"⚠️ File is too large. Just send me a file less than {max_display_gb:.1f}GB.")
        return
